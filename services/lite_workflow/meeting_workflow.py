...
def parse_ocr_results_for_workflow(
    ...
    for item in captures:
        ...
        ocr_segments.append(
            TranscriptSegment(
                speaker="ข้อมูลหน้าจอประกอบการประชุม",
                text=segment_text,
                start=ts_sec,
                end=ts_sec + 0.4,
            )
...
class KnowledgeGraph:
    ...
    @classmethod
    def load_from_state(cls, state: Dict[str, Any]) -> "KnowledgeGraph":
        kg = cls()
        if "kg" not in state or not isinstance(state["kg"], dict):
            state["kg"] = {"nodes": {}, "edges": [], "edge_attrs": {}}
        kg.nodes = state["kg"].setdefault("nodes", {})
        kg.edges = state["kg"].setdefault("edges", [])
        kg.edge_attrs = state["kg"].setdefault("edge_attrs", {})
        return kg
    ...
    def add_node(self, node_id: str, payload: Dict[str, Any]) -> None:
        ...
        if self.nodes[node_id].get("type") == "topic" and payload.get("type") == "topic":
            ...
            if payload.get("evidence"):
                ...
                # Cap the number of evidence lines stored to save memory
                merged_ev = sorted(list(old_ev | add_ev))
                self.nodes[node_id]["evidence"] = merged_ev[:15]
    ...
    def query_agenda(self, agenda_title: str) -> Dict[str, Any]:
        ...
        import re
        m = re.match(r'^(?:วาระที่\s*)?(\d+)', str(agenda_title).strip())
        agenda_prefix = f"{m.group(1)}." if m else None

        for nid, node in self.nodes.items():
            if node.get("type") == "topic":
                title = str(node.get("title", "")).strip()
                if agenda_prefix and title.startswith(agenda_prefix):
                    topic_ids.add(nid)
...
ต้องตอบเป็น JSON เท่านั้น:
{
  ...
  "dept_context": "ฝ่ายหรือแผนกที่เกี่ยวข้องกับบทสนทนานี้",
...
"""
    ...
    def _empty_extract_result(self, parse_failed: bool = False) -> Dict[str, Any]:
        data: Dict[str, Any] = {"mentioned_names": [], "dept_context": "", "speakers": [], "topics": [], "actions": [], "decisions": []}
        if parse_failed:
    ...
    def _dedup_topics(self, topics: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for t in topics:
            ...
            ngrams = char_ngrams(title, n=4)
            if not ngrams:
                if title:  # Keep if it has some non-n-grammable title
                    merged.append(t)
                continue
            
            is_dup = any(jaccard(ngrams, prev) > 0.6 for prev in seen_ngrams)
            if not is_dup:
                merged.append(t)
    ...
    def _merge_extract_payloads(self, payloads: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for payload in payloads:
            ...
            # merge mentioned_names and dept_context
            if isinstance(payload.get("mentioned_names"), list):
                merged["mentioned_names"].extend(payload["mentioned_names"])
            if payload.get("dept_context"):
                if merged["dept_context"]:
                    merged["dept_context"] += " / " + payload["dept_context"]
                else:
                    merged["dept_context"] = payload["dept_context"]
                    
        merged["topics"] = self._dedup_topics(merged.get("topics", []))
        # deduplicate mentioned_names
        merged["mentioned_names"] = list(set(merged.get("mentioned_names", [])))
        return merged
    ...
    async def __call__(self, state: "MeetingState") -> "MeetingState":
        ...
        def coerce_title(t: Optional[str]) -> Optional[str]:
            ...
            if t in agenda_titles:
                return t
            return best_fuzzy_match(t, agenda_titles, threshold=0.50)
...
class GeneratorAgent:
    ...
    def __init__(self, client: TyphoonClient):
        ...
        self.narrative_system = """คุณคือเลขานุการการประชุมองค์กรระดับมืออาชีพ

ก่อนเขียนรายงาน ให้ทำ 2 ขั้นนี้กับ evidence ที่ได้รับ:

ขั้นที่ 1 — กรองและทำความสะอาด evidence แต่ละบรรทัด:
- ตัดคำติดปากและ filler ออก: เอ่อ / อ้า / นะครับ / นะคะ / ครับ / ค่ะ / เนาะ
- รวมคำที่ซ้ำกันติดต่อกันให้เหลือแค่ครั้งเดียว เช่น "จริง จริง" -> "จริง", "จริง จริง จริง" -> "จริง"
- ถ้า evidence บรรทัดใดเป็นแค่คำพูดไม่มีสาระ ให้ข้ามบรรทัดนั้นทิ้ง

ขั้นที่ 2 — สรุปใจความแต่ละ evidence ก่อนใส่รายงาน:
- อย่า copy-paste evidence ลงรายงานตรงๆ
- แปลงเป็นประโยคภาษาเขียนทางการ 1 ประโยคต่อ 1 ใจความ
- รักษาตัวเลข ชื่อโครงการ ชื่อบุคคล ให้ครบถ้วน

แล้วจึงเขียนรายงานจาก evidence ที่ผ่านทั้ง 2 ขั้นแล้วเท่านั้น

กติกาผลลัพธ์:
- เขียนเป็นร้อยแก้วภาษาราชการ ไม่มี table / bullet / JSON
- ห้ามเพิ่มข้อมูลที่ไม่มีใน evidence
- ห้ามอ้างถึง "อ้างอิงจากหน้าจอ" หรือรหัส [#id] ใดๆ ในเนื้อหา"""

        self.extractor_system = """คุณคือระบบ structured data extractor
หน้าที่เดียวของคุณคือดึงข้อมูลจาก prose ที่ได้รับแล้วแปลงเป็น JSON ตาม schema ที่กำหนด

กติกาบังคับ:
- ห้ามเพิ่ม แก้ไข หรือตีความข้อมูลใหม่เกินกว่าที่อยู่ใน prose
- ถ้า prose ไม่มีข้อมูลใด ให้ใส่ [] หรือ "" ใน field นั้น อย่าเดา
- ตอบเป็น JSON เท่านั้น ไม่มีคำอธิบายใดๆ นอก JSON"""
    ...
    def _split_agenda_if_needed(self, ag: AgendaItem) -> List[AgendaItem]:
        sub_items = [d for d in ag.details or [] if re.match(r'^\d+\.\d+', d.strip())]
        if len(sub_items) >= 4:
            subs = []
            for sub in sub_items:
                subs.append(AgendaItem(title=f"{ag.title} ({sub})", details=[sub]))
            return subs
        return [ag]
    ...
    def _clean_html(self, text: str) -> str:
        ...
        text = re.sub(r"\[OCR\s+\d{2}:\d{2}:\d{2}\]", "", text, flags=re.IGNORECASE)
        text = re.sub(r"ข้อมูลหน้าจอประกอบการประชุม", "", text, flags=re.IGNORECASE)
        text = re.sub(r"อ้างอิงจากหน้าจอเวลา\s*\[\d{2}:\d{2}:\d{2}\]:?", "", text, flags=re.IGNORECASE)
        return text.strip()
    ...
    def _collect_ocr_evidence_lines(self, agenda: AgendaItem, ocr_captures: List[Dict[str, Any]]) -> List[str]:
        ...
        for score, cap, hits, rare_entities in scored[: self.ocr_max_evidence_lines]:
            ...
            lines.append(f"อ้างอิงจากหน้าจอเวลา [{ts_hms}]: {txt}")
        return lines
    ...
    def _build_narrative_prompt(
        ...
        compact = {
            ...
            "topics": [
                {"title": t.get("title"), "details": (t.get("details") or "")[:450]}
                for t in topics
            ][:self.max_followup_rows],
            "decisions": [
...
ข้อกำหนด:
- เขียนเป็นร้อยแก้วต่อเนื่อง ครอบคลุมหัวข้อที่อภิปราย มติที่ประชุม และงานที่มอบหมาย
...
{{
  ...
  "actions": [
    {{"task": "งานที่ต้องทำ", "owner": "ผู้รับผิดชอบ", "due": "กำหนดการ", "note": "หมายเหตุ"}}
  ]
}}

กติกา:
- summary_points ต้องมี 6-12 รายการ แต่ละรายการเป็นประโยคสมบูรณ์
- ดึงเฉพาะข้อมูลที่มีใน narrative ห้ามเพิ่มหรือเดา
- ถ้าไม่มี decisions หรือ actions ให้ใส่ []
...
{narrative}"""
    ...
    def _render_html(self, structure: Dict[str, Any], agenda_title: str) -> str:
        ...
        # ── ตารางติดตาม ───────────────────────────────────────────────────
        parts.append("<h4>ตารางติดตาม</h4>")
        followup = structure.get("followup_rows") or []
        if followup:
            rows = ""
            for row in followup:
                dept   = html_lib.escape(str(row.get("department") or "ผู้เกี่ยวข้อง"))
        ...
        # ── มติที่ประชุม ──────────────────────────────────────────────────
        parts.append("<h4>มติที่ประชุม</h4>")
        ...
        # ── Action Items ──────────────────────────────────────────────────
        parts.append("<h4>Action Items</h4>")
        actions = structure.get("actions") or []
        if actions:
            rows = ""
            for a in actions:
                task  = html_lib.escape(str(a.get("task") or "-"))
    ...
    async def _generate_narrative(
        ...
        messages = self._build_narrative_prompt(agenda, agenda_data, evidence_text)
        resp = await self.client.generate(
            messages,
            temperature=0.3,
            completion_tokens=stage_completion_tokens("GEN_NARRATIVE_COMPLETION_TOKENS", 2400),
            auto_continue=True,
        )
    ...
    async def _extract_structure(self, narrative: str, agenda_title: str) -> Dict[str, Any]:
        ...
        resp = await self.client.generate(
            messages,
            temperature=0.05,
            completion_tokens=stage_completion_tokens("GEN_EXTRACT_COMPLETION_TOKENS", 1800),
        )
        data = try_parse_json(resp)
        if not data:
            # repair pass
    ...
    async def __call__(self, state: "MeetingState") -> "MeetingState":
        ...
        async def gen_one(i: int, ag: AgendaItem) -> Tuple[int, str]:
            async with sem:
                sub_agendas = self._split_agenda_if_needed(ag)
                html_parts = []
                for sub_ag in sub_agendas:
                    agenda_data = kg.query_agenda(sub_ag.title)
                    ids = self._collect_evidence_ids(sub_ag, agenda_data, transcript_index)
                    evidence_text = self._build_evidence_text(transcript_index, ids)
                    ocr_lines = self._collect_ocr_evidence_lines(sub_ag, ocr_captures)
                    if ocr_lines:
                        evidence_text = evidence_text + "\n\nOCR EVIDENCE:\n" + "\n".join(ocr_lines)

                    logger.info(
                        "Generate %d/%d (sub): agenda=%s (evidence_ids=%d, ocr_lines=%d, evidence_chars=%d)",
                        i + 1, len(agendas), sub_ag.title, len(ids), len(ocr_lines), len(evidence_text)
                    )
                ...
                if len(sub_agendas) > 1:
                    merged_html = ""
                    for sub_ag, html in zip(sub_agendas, html_parts):
                        merged_html += f"<h4>เรื่อง: {sub_ag.title}</h4>\n{html}\n<hr/>\n"
                    return i, merged_html
                else:
                    return i, html_parts[0]
...
7) เขียนด้วยภาษาเขียนที่เป็นทางการ ห้ามใช้ภาษาพูด (เช่น เอ่อ, อ้า, เนาะ, นะคะ, นะครับ, จริงๆ, แบบว่า, คือ) โดยเด็ดขาด
8) ห้ามใส่คำว่า "อ้างอิงจากหน้าจอ" หรืออ้างอิงถึงเอกสารดิบโดยตรง ให้สรุปเป็นเนื้อหาทางการ
9) ตอบเป็น HTML fragment เท่านั้น
""",
            },
        ]
        resp = await self.client.generate(
...
class OfficialEditorAgent:
    ...
    def __init__(self, client: TyphoonClient):
        self.client = client
        self.enabled = env_flag("OFFICIAL_EDITOR_ENABLED", True)
        self.max_parallel = int(os.getenv("OFFICIAL_EDITOR_MAX_PARALLEL", "2"))
        self.completion_tokens = stage_completion_tokens("OFFICIAL_EDITOR_COMPLETION_TOKENS", 4000)
    ...
    def _collect_ocr_evidence_lines(self, agenda: AgendaItem, ocr_captures: List[Dict[str, Any]]) -> List[str]:
        ...
        for score, cap, hits, rare_entities in scored[: max(0, self.ocr_max_evidence_lines)]:
            ...
            lines.append(f"อ้างอิงจากหน้าจอเวลา [{ts_hms}]: {text}")
        return lines
...
- ห้ามคัดลอกคำว่า "อ้างอิงจากหน้าจอเวลา" หรือ "ข้อมูลหน้าจอประกอบการประชุม" มาใส่รายงานเด็ดขาด (ใช้อ่านประกอบเท่านั้น)
        ...
        for i, agenda in enumerate(agendas):
            ...
            for cap in caps:
                ...
                for tid in topic_ids:
                    ...
                    # Increased threshold from min_overlap to max(0.2, min_overlap) to prune weak edges
                    if ov >= max(0.2, min_overlap):
...
def build_workflow() -> Any:
    ...
    graph.add_node("polish_transcript", TranscriptPolishAgent(client))
    ...
    graph.add_edge("augment_with_ocr", "polish_transcript")
    graph.add_edge("polish_transcript", "extract_kg")
